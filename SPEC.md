...
| Signed installers + updater | MSI/NSIS, notarized .dmg, .deb/.rpm, and a signed update feed. Azure Trusted Signing for Windows, Apple Developer notarization for macOS. Shell logic already lives behind a thin binary so a bundler can wrap it without a rewrite | planned | later |
...
| CI | Build + unit tests on Node 22 and 24, the HTTP and feature smokes, `cargo fmt`/`clippy`/`test` on Linux, macOS and Windows, and the packaged-install smoke | shipped | v0.9.0 |
