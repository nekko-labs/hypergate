...
| ~~Electron shell~~ | Superseded in v0.8.0: the desktop presence is a native Rust tray agent plus the existing web UI in the user's browser, not a bundled webview (§3.8) | superseded | later |
...
| Browser-based UI | "Open manager" opens the existing web UI in the user's **default browser**. No bundled webview: developers already have a browser, it is a better web runtime than any embedded one, and it avoids a hard `webkit2gtk` dependency on Linux for a window we would never render | shipped | v0.8.0 |
...
| CLI | `hypergate start\|stop\|restart\|status\|list\|logs\|open\|gateway\|tray\|autostart\|secret\|sandbox-exec`, all talking to the daemon's existing HTTP API with the gateway token. One binary to install, sign and put on `PATH` | shipped | v0.8.0 |
...
| Signed installers + updater | Code signing and a signed update feed on top of the installers below. Azure Trusted Signing for Windows, Apple Developer notarization for macOS. Until then an installer download hits SmartScreen and Gatekeeper warnings | planned | later |
