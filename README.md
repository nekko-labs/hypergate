...
**Claude Desktop** gets a double-clickable extension instead: `npm run build:mcpb` produces `dist-mcpb/hypergate.mcpb`, which Desktop installs from Settings → Extensions. Where Hypergate can be listed in Claude's own surfaces — and why the remote-connector directory is not one of them — is [docs/listing-submissions.md](docs/listing-submissions.md). It runs the daemon in `--stdio` mode, so it attaches to the gateway you already have running rather than starting a second copy of every server, and it shows up in **Connected agents** as *Claude Desktop*, revocable on its own.
